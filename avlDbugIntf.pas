...
uses
  Windows, AvL{Dialogs}; // We need "Dialogs" for TMsgDlgType
...
{  with TRegIniFile.Create('\Software\GExperts') do
  ...
  try
    RegKeyOpenRead(HKEY_CURRENT_USER, 'SOFTWARE\GExperts\Debug');
    DebugFileName:=RegKeyGetStr(Key, 'FilePath');
  finally
    RegKeyClose(Key);
  end;
