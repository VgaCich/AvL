...
interface
uses
  Windows, Messages, WinSock, AvlICQWorks, AvlICQSock, AvlICQDirect2, AvlICQLang, Avl;
...
type
  ...
  TOnOnlineInfo = procedure(Sender: TObject; UIN: String; Port: Word; InternalIP, ExternalIP: String; ProtoVer, UserCaps: Byte) of object;
  ...
  TOnAuthRequest = Procedure(Sender:TObject; UIN:String; Var Reason:String; Var Authorize:Boolean) of Object;
  ...
  private
    ...
    FProxyResolve: Boolean;
    //-- Events & other stuff --
    FContactLst: TStringList;
    FVisibleLst: TStringList;
    FInvisibleLst: TStringList;
    FOnMsg: TOnMsgProc;
    FOnURL: TOnURLProc;
    ...
    {$HINTS ON}    
    procedure SetContactList(Value: TStringList);
    procedure SetVisibleList(Value: TStringList);
    procedure SetInvisibleList(Value: TStringList);
  ...
  published
    ...
    property ContactList: TStringList read FContactLst write SetContactList;
    property VisibleList: TStringList read FVisibleLst write SetVisibleList;
    property InvisibleList: TStringList read FInvisibleLst write SetInvisibleList;
...
added to ContactList TStringList. After adding the UIN you will receive status
...
added to ContactList TStringList. After adding the UINs you will receive status
...
begin
  ...
  if Assigned(OnOnlineInfo) then
    FOnOnlineInfo(Self, UIN, FIntPort, NumToIp(FIntIP), NumToIp(FExtIP), FProtoVer, UserCaps);
end;
{$WARNINGS ON}
...
begin
  ...
  If Assigned(OnAuthRequest) Then Begin
    fOnAuthReq(Self, FUIN, FReason, fAuth);
    If fAuth Then
      CreateCLI_AUTHORIZE(@opkt, StrToInt(FUin), 1, '', FSeq)
    Else
      CreateCLI_AUTHORIZE(@opkt, StrToInt(FUin), 0, FReason, FSeq);
...
var
  ...
  begin
    ...
    //Group header
    if (FGroup <> '') and (CTag <> 0) and (CId = 0) then
      GroupIdents.Values[IntToStr(CTag)] := UTF8ToStrSmart(FGroup);

   //UIN entry
    if (CType = 0) or (CType=1) or (CType = 2) or (CType = 3) or (CType = $e) then
    begin
      GetMem(lpEntry, SizeOf(lpEntry^));
      if CType<>1
         then lpEntry^.UIN := StrToIntDef (FGroup, 0) // *yegor thx to Jeka
         else lpEntry^.UIN:=CTag;
      lpEntry^.Nick := FNick;
      lpEntry^.CType := CType;
      lpEntry^.CTag := CId;
...
procedure TICQClient.SetContactList(Value: TStringList);
begin
  FContactLst.Assign(Value);
end;

procedure TICQClient.SetVisibleList(Value: TStringList);
begin
  FVisibleLst.Assign(Value);
end;

procedure TICQClient.SetInvisibleList(Value: TStringList);
begin
