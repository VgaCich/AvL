...
{  PCX - PC Paintbrush format (ZSoft) support for KOL.
  ...
  Adaptation for Avl - Avenger - 18.03.04
  E-Mail: avenger@nht-team.org
  WWW:    http://www.nht-team.org
}
...
var Strm: TStream;
begin
  Strm := TResourceStream.CreateFromID(Instance, ResID, RT_RCDATA);
  Strm.Position := 0;
  Result := LoadFromStream( Strm );
  Strm.Free;
end;
...
var Strm: TStream;
begin
  Strm := TResourceStream.Create(Instance, Name, RT_RCDATA);
  Strm.Position := 0;
  Result := LoadFromStream( Strm );
  Strm.Free;
end;

function TPCX.LoadFromStream(Stream: TStream): Boolean;
type
...
var
  StartPos: DWORD;
    ...
    begin
      ...
      FBitmap := TBitmap.CreateNewDIB(W, H, Format);
