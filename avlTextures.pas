...
function LoadTexture(Filename: String; var Texture : GLuint; LoadFromRes : Boolean; Level: Byte; Formt: Cardinal) : Boolean;
...
{$endif}
{  S:=gluErrorString(gluBuild2DMipmaps(GL_TEXTURE_2D, 4, Width, Height, Format, GL_UNSIGNED_BYTE, pData));
  if S<>'' then exit;}
//  if glGetError<>0 then exit;
  result :=Texture;
end;
...
begin
  ...
  {$ifdef AvL}
  JPG.Draw(BMP.canvas, Rect(0, 0, JPG.Width, JPG.Height));
  {$else}
  ...
  Begin
    ...
    Begin
      ...
      G:=(c shr 16) and $FF;
      R:=(c shr 32) and $FF;
      Data[W+(H*Width)] :=(((c and $FF) shl 16)+(c shr 16)+(c and $FF00));  // 4 channel.
//      if c=0 then Data[W+(H*Width)]:=Data[W+(H*Width)] and $00FFFFFF;
      if r>g then begin
        if r>b then a:=r else a:=b;
      end else if g>b then a:=g else a:=b;
      if a<L then begin
              a:=a shl 48;
      end else a:=$FF000000;
      Data[W+(H*Width)]:=Data[W+(H*Width)] or A;
      inc(Line);
    End;
  End;
  ...
  Texture :=CreateTexture(Width, Height, Fmt, addr(Data[0]));
  result :=TRUE;
end;
...
begin
  L:=Level;
  //if Formt>0 then Fmt:=Formt else
  Fmt:=GL_RGBA;
